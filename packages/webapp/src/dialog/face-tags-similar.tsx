...
export interface SimilarTagsProps{
    tags: string[],
    addTag: (tag: string) => void,
}

const Tags: FunctionComponent<SimilarTagsProps> = ({tags, addTag}) => {
  return (
    <>{tags.map((tag, i) => (
...
export const SimilarTags = ({tags, dispatch}: {tags: string[], dispatch: Function}) => {
  ...
  return (
    <>
      ...
      <p className="flex flex-wrap items-center justify-start gap-2">
        <Tags tags={tags.slice(0, tagCount).sort()} addTag={tag => dispatch({type: 'addTag', value: tag})} />
        { tagCount < tags.length &&
          <a
            className="text-gray-500 hover:text-gray-300 hover:cursor-pointer"
            onClick={() => setTagCount(count => 2 * count)}>show more...
          </a>
        }
      </p>
    </>
