  ...
  function extractFrames(entry, cb) {
    ...
    command
      ...
      .on('end', () => {
        ...
        resizeImage(storageDir, entry, path.join(storageDir, thumbFilenames[0]), previewImageSizes, (err, resizeCount) => {
          if (!err && resizeCount) {
            debug(`Created ${resizeCount} preview video images from ${entry.filename}:${entry.sha1sum} with sizes of ${previewImageSizes.join(',')}`)
          }
          cb(err);
        });
      })
