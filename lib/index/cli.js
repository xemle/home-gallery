  ...
  handler: (argv) => {
    fileFilter(argv.exclude, argv['exclude-from-file'], (err, fileFilterFn) => {
      if (err) {
        ...
        update(argv.directory, argv.index, argv.checksum, fileFilterFn, () => true)
      }
    })
  }
}

const createOrUpdate = (base, indexFilename, fileFilterFn, cb) => {
  const now = new Date();
  async.waterfall([
    (callback) => readIndex(indexFilename, callback),
    (fileIndex, callback) => {
      createIndex(base, fileFilterFn, (err, fsEntries) => {
        if (err) {
          return callback(err);
        }
...
const update = (base, indexFilename, calculateChecksum, ignoreFn, cb) => {
  const t0 = Date.now();
  async.waterfall([
    (callback) => createOrUpdate(base, indexFilename, ignoreFn, callback),
    (index, callback) => {
